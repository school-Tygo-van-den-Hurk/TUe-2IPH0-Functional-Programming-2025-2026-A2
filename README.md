...
- [ ] At least two sensible tests exist per function in [`test/Spec.hs`][test]. For an excellent grade, more thorough testing is needed.
- [ ] Names and student numbers are current at the top of [`src/PegSolitaire.hs`][src] and [`test/Spec.hs`][test]
...
- [Haskell Language Reference][haskell]
- [Data.Tree documentation][data.tree] for `Tree` type and utilities
- [Data.List documentation][data.list] especially `unfoldr` and `catMaybes`
- [Data.Maybe documentation][data.maybe]
- [Hoogle][hoogle]: search for Haskell functions
...
Define function `foldT`, the catamorphism factory for the `Tree` type. Compare this to `foldr` for lists and `foldTree` (from [`Data.Tree`][data.tree]).

#### Exercise 3: `Zipper`, `toZipper`, `fromZipper`, `tryRight`, `tryLeft`

We can store the normal one-dimensional game state in a list. However, we can only access the first element in a list in constant time. Another way to do this is to introduce a `Zipper` structure, we can do this for most data structures, but in this assignment, we focus on the zipper of a list. In a Zipper, you 'walk' through the data structure current. You store the current value under focus, the remainder of the data structure, and a left of how you walked through the structure. For a list, you go from left to right through the list. So the focus is the current value, the remainder of the data structure is the list of all values to the right of the focus, and the left is all the values left of the focus. When you store the left in reverse order, you can prepend the focus when moving to the right.

For instance the list `[1,2,3,4,5]`, we can have `3` as focus. Then, `[4,5]` is the remainder and `[2,1]` is the left. So when we go one place to the right, we get `4` as new focus, `[5]` as a new remainder and `[3,2,1]` as new left.
...
Given `n` of type `Int`, define the following functions using [`unfoldr`][unfoldr] (from `Data.List`).

##### generateLinearStates

This function should give all possible states with `n-1` positions filled with pegs and one empty position. E.g. `generateLinearStates 2` should give the elements `X .` and `. X`.

##### generateStates

This function gives a list of all possible states up to size `n`. E.g. `generateStates 2` should give the elements `.`, `X`, `. .`, `X .`, `. X` and `X X`.

> [!TIP]
> There are several ways to go for `generateStates`; one such way uses tupling.

> [!NOTE]
> Use these functions in the remainder of the Assignment to have a broad test set.
...
> [!TIP]
> Consider using [`catMaybes`][catMaybes] from `Data.Maybe` to collect valid moves.
...
> [!NOTE]
> The first argument of `unfoldT` should be a function that returns a suitable `Either` type. The second argument should be the seed.
...
> [!NOTE]
> Its input types should be regular lists, not zippers.
...
> [!NOTE]
> We do not specify what this sequence of moves should look like. Define a suitable data type yourself that fits here. The input types of these functions should be regular lists, not zippers.
>
> Additionally, the following should be a validly typed expression: `(\ps -> trySolution ps (=<<) getSolution ps)`. (See [`=<<`][=<<].)

> [!IMPORTANT]
> Do not break previously defined functions when working on this exercise.

[hoogle]: https://hoogle.haskell.org/
[haskell]: https://www.haskell.org/

[data.list]: https://hackage.haskell.org/package/base-4.17.2.1/docs/Data-List.html
[data.maybe]: https://hackage.haskell.org/package/base-4.17.2.1/docs/Data-Maybe.html
[data.tree]: https://hackage.haskell.org/package/containers-0.6.6/docs/Data-Tree.html

[catMaybes]: https://hackage.haskell.org/package/base-4.17.2.1/docs/Data-Maybe.html#v:catMaybes
[=<<]: https://hoogle.haskell.org/?hoogle=%3D%3C%3C&scope=set%3Astackage
[unfoldr]: https://hackage.haskell.org/package/base-4.17.2.1/docs/Data-List.html#v:unfoldr

[src]: ./src/PegSolitaire.hs
[test]: ./test/Spec.hs
